...
## 1. `src/blowfish.cpp` – `pkcs7Unpad()` underflow / out-of-bounds read (lines 216–220)

**File:** `src/blowfish.cpp`
...
## 2. `src/blowfish.cpp` – `pkcs7Pad()` unreachable dead branch (lines 199–201)
...
## 3. `src/blowfish.cpp` – `decryptBlock()` does not correctly invert `encryptBlock()` (lines 243–253)
...
## 4. `src/blowfish.cpp` – `decryptText()` missing input length validation (lines 52–60)
...
## 5. `src/blowfish.cpp` – `blocksToBytes()` missing capacity reservation (lines 139–157)
...
## 6. `src/blowfish.h` – Constructor docstring mentions non-existent `key_len` parameter (lines 13–15)
...
## 7. `src/blowfish.h` – Public API doesn't document ECB mode (lines 28–38)
...
## 8. `src/crypto_bf_bf_pi.h` – Missing include guard (lines 16–20)

**File:** `src/crypto_bf_bf_pi.h`
...
## 10. `tests/test_blowfish.cpp` – Missing `#include <cstdlib>` (lines 1–15)

**File:** `tests/test_blowfish.cpp`
...
## 11. `tests/test_blowfish.cpp` – `#define private public` is brittle (lines 8–10)
...
## 12. `tests/test_blowfish.cpp` – Noisy DEBUG prints during normal test execution (lines 95–101)
...
## 13. `tests/test_blowfish.cpp` – Missing regression test for `padLen > input.size()` (lines 60–77)
