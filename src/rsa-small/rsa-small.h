...
public:
    ...
    // Binary-oriented API
    static std::vector<uint8_t> encrypt(const std::vector<uint8_t>& plaintext, const key& publicKey);
    static std::vector<uint8_t> decrypt(const std::vector<uint8_t>& ciphertext, const key& privateKey);

    // Text-oriented API
    /**
     * Encrypts a plaintext string and returns the ciphertext as a vector of bytes.
     */
    static std::vector<uint8_t> encryptText(const std::string& plaintext, const key& publicKey);

    /**
     * Decrypts a ciphertext represented as a vector of bytes and returns the resulting plaintext string.
     */
    static std::string decryptText(const std::vector<uint8_t>& ciphertext, const key& privateKey);
