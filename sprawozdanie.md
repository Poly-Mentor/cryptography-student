...
Link do repozytorium z kodem źródłowym i instrukcją: [https://github.com/Poly-Mentor/cryptography-student]()
...
**Inicjalizacja P i S**: Początkowe wartości P i S pobrane z pliku `crypto_bf_bf_pi.h` (wartości wyprowadzone z cyfr liczby pi — nagłówek pochodzi z biblioteki OpenSSL). Następnie wykonywana jest inicjalizacja kluczy (***key schedule***): bajty klucza są powtarzane i XOR-owane z elementami `P` , a potem szyfruje się blok zerowy wielokrotnie, aby nadpisać kolejne elementy `P` i `S` (funkcja `initialize` ).
...
Implementacja jest zgodna z RFC 1321: przetwarzanie 512-bitowych bloków (64 bajtowych) bloków, wynikiem jest 128-bitowy (16 bajtowy) hash.
...
**8. Bonus - kompilacja do WebAssembly**

Byłem ciekawy możliwości kompilacji projektu C/C++ do WebAssembly, więc dodatkowo, jako element niepodlegający ocenie, przy pomocy agenta AI, dodałem prosty wrapper ( `src/wasm/wasm_wrapper.cpp` ) z funkcjami `extern "C"` do eksportu, które przyjmują wskaźniki i rozmiary buforów, konwertują je na `std::vector<uint8_t>` , wywołują odpowiednie algorytmy, a następnie kopiują wynik z powrotem do bufora wyjściowego. Następnie skompilowałem projekt przy użyciu Emscripten ( `emcc` ). Agent AI wygenerował mi stronę HTML/JavaScript ( `wasm_demo/index.html` ), która ładuje skompilowany moduł WebAssembly i pozwala użytkownikowi wprowadzić dane do zaszyfrowania/odszyfrowania lub haszowania, a następnie wyświetla wynik. To pozwala na interaktywną demonstrację działania algorytmów bezpośrednio w przeglądarce, co jest, jak sądzę, ciekawym dodatkiem do projektu, pozwalającym przetestować działanie algorytmów w przeglądarce bez kompilacji źródeł, na dowolnej platformie.
...
Testy jednostkowe znajdują się w katalogu `tests/` (np. [tests/blowfish/test_blowfish.cpp](tests/blowfish/test_blowfish.cpp)). Można je uruchomić za pomocą `make run_tests` lub bezpośrednio wykonując skompilowane pliki testowe (np. `./tests/blowfish/test_blowfish` ).
