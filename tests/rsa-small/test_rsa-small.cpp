...
int main() {
    ...
    for (uint8_t key_len = 9; key_len <= 48; key_len++) {
        ...
        std::vector<uint64_t> ciphertext = RSA::encrypt(data, keys.publicKey);
        std::vector<uint8_t> decrypted = RSA::decrypt(ciphertext, keys.privateKey);

        std::string test_case_name = "RSA encryption/decryption with " + std::to_string(key_len) + "-bit keys";
        if (decrypted == data) pass(test_case_name);
